(function(global, $, _, _m, thisPage) {
    ...
    const _MY_NETWORK = thisPage.myNetwork;
    const _PARTNER_NETWORK = thisPage.partnerNetwork;
    const _DIRECTION = thisPage.direction;
    const _BTN_ROLE = thisPage.btnRole;
    let _MSZI = 99; // multi selector z-index init max
    let _COMMON_CODES = {};
    let _BAS_CALL_TYPES = [];
    let _TMP_SPCL_ID = '';
    let _TAB_DIRECTION = 'IB'; //(Inboud, Outbound)
    let _CONTRACT_STATUS = 'Working'; // (Working, Draft, Agreed, Rejected)
    let _DATE_PICKER_INIT = false;
    
    /**
     * Contract Domain
     */
    let _CONT_MSTR_DOMAIN = new ContMstr(); //(Agreement)
    let _CONT_DTL_DOMAIN = new ContDtl(); // (Contract)
    ...
    function findCommonCodes() {
        ...
        $.ajaxProxy($.reqGet(url).build()).then(function(res) {
            ...
            //let thrsTypeCodes = res.data['thrsTypeCodes']; // Change Rate, Fixed Charge, Special Rule
    ...
    function initDisplayContractDetailListByPlmns() {
        let url = _CTX + "dch/contract/findContractsByPlmns";
        let params = {};
        params.trmPlmnId = _MY_NETWORK;
        params.rcvPlmnId = _PARTNER_NETWORK;
        params.direction = _DIRECTION;
        
        $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
            let result = response['data'];
            
            if (!result.contId || _.isEmpty(result.contDtls)) {
                global.alert(thisPage.appMsg001);
                global.location.replace(_CTX + "dch/contract/contractList");
                return;
            }
            
            displayContractList(result);
        });
        
    }
    
    function changeTabTitleButtonDisplay(tabDirection, contractStatus) {
        
        if (tabDirection === 'IB' && contractStatus === 'Working') {
            // Tab
            $('#contTypeDirection').text('Inbound');
            $('#calInfo').text("IB " + thisPage.appMsg055);
            
            // Button
            $('#saveTempContract').show();
            $('#requestConsent').show();
            $('#responseConset').hide();
            $('#responseRefusal').hide();
            
        } else if (tabDirection === 'IB' && contractStatus !== 'Working') {
            // Tab
            $('#contTypeDirection').text('Inbound');
            $('#calInfo').text("IB " + thisPage.appMsg055);
            
            // Button
            $('#saveTempContract').hide();
            $('#requestConsent').hide();
            $('#responseConset').hide();
            $('#responseRefusal').hide();
            
        } else if (tabDirection === 'OB' && contractStatus === 'Draft') {
            // Tab
            $('#contTypeDirection').text('Outbound');
            $('#calInfo').text("OB " + thisPage.appMsg055);
            
            // Button
            $('#saveTempContract').hide();
            $('#requestConsent').hide();
            $('#responseConset').show();
            $('#responseRefusal').show();
            
        } else if (tabDirection === 'OB' && ( contractStatus === 'Agreed' || contractStatus === 'Rejected' )) {
            // Tab
            $('#contTypeDirection').text('Outbound');
            $('#calInfo').text("OB " + thisPage.appMsg055);
    ...
    function displayContractList(contMaster) {
        // Contract Master Info
        let myNetwork = contMaster.trmPlmnId;
        let partnerNetwork = contMaster.rcvPlmnId;
        let contType = getConTypeCodeValue(contMaster.contTypeCd);
        let contrId = contMaster.contrId;
        $('#trmPlmnId').val(myNetwork);
        $('#rcvPlmnId').val(partnerNetwork);
        $('#contTypeCd').val(contType);
        $('#contrId').val(contrId);
        
        // Contract List
        let contDtlHtml = '';
        contMaster.contDtls.forEach(contDtl => {
            contDtlHtml += '<tr class="contList" data-cont-id="' + contDtl.contId + '" data-cont-dtl-id="' + contDtl.contDtlId + '" >';
            contDtlHtml += '<td>' + contDtl.direction + '</td>';
            contDtlHtml += '<td>' + getContSttusCodeValue(contDtl.contSttusCd) + '</td>';
            contDtlHtml += '<td>' + $.localDateFormat(contDtl.contStDate) + '</td>';
            contDtlHtml += '<td>' + $.localDateFormat(contDtl.contExpDate) + '</td>';
            contDtlHtml += '<td>' + $.abbreviate(contDtl.contDtlMemo, 30) + '</td>';
            contDtlHtml += '</tr>';
        });
        
        _CONT_MSTR_DOMAIN = contMaster;
        _CONT_MSTR_DOMAIN.contDtls = [];
        $('#contractList').html(contDtlHtml);
        
    }
    

    function getConTypeCodeValue(contType) {
        return {
            IOBR: 'IB/OB Roaming',
            IBR: 'IB Roaming',
            OBR: 'OB Roaming'
    ...
    function getContSttusCodeValue(contSttusCd) {
        return {
            WRK: 'Working',
            DRF: 'Draft',
            AGR: 'Agreed',
    ...
    function findContractDetailByContIdAndConDtlId(contId, contDtlId) {
        let url = _CTX + "dch/contract/findContractDetailByContIdAndConDtlId";
        let params = {};
        params.contId = contId;
        params.contDtlId = contDtlId;
        
        $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
            let result = response['data'];
            
            _CONT_DTL_DOMAIN = result;
            
            displayContractBaseInfoPanel(result);
            
            displayContractBaseTarifPanel(result);
            
            displayContractSpecialModelPanel(result);
            
        });
        
    }
    
    function showContractDetailDivs() {
        $('#contractBaseInfo').show();
        $('#contractIbInfo').show();
        $('#contractBaseTarifInfo').show();
        $('#contractSpecialInfo').show();
    }
    
    function displayContractBaseInfoPanel(contract) {
        // initialize contract date
        $('#contStDate').val(_m(addDays()).format('YYYY-MM-DD'));
        $('#contExpDate').val(_m(addDays()).format('YYYY-MM-DD'));
        $('#contExpNotiDate').val(_m(addDays()).format('YYYY-MM-DD'));
        
        let contStDate = _m(contract.contStDate, ["YYYY-MM-DD"]).toDate();
        let contExpNotiDate = _m(contract.contExpNotiDate, ["YYYY-MM-DD"]).toDate();
        let contExpDate = _m(contract.contExpDate, ["YYYY-MM-DD"]).toDate();
        
        setContractNewEndDate(contExpDate);
        setContractNewEndNotiDate(contExpNotiDate);
        setContractNewStartDate(contStDate);
        
        _DATE_PICKER_INIT = true;
        
        // contract auto update
        $('.contAutoUpdYnOpts').remove(); 
        let contAutoUpdYnCodes = _COMMON_CODES.contAutoUpdYnCodes;
        let defaultAutoUpdYnOption = contract.contAutoUpdYn; // contract auto yes
        let contAutoUpdYnOption = contAutoUpdYnCodes.map(function(contAutoUpdYnCode) {
            return { label : contAutoUpdYnCode.cdVal1, value : contAutoUpdYnCode.cdId }
        });
        
        initSingleSelectbox('#contAutoUpdYn', 'contAutoUpdYnOpts', contAutoUpdYnOption, defaultAutoUpdYnOption);
        
        let contDtlMemo = contract.contDtlMemo;
        $('#contDtlMemo').val(contDtlMemo);
        
        /** IB **/
        let currencyCodes = _COMMON_CODES.currencyCodes;
        let defaultCurrency = contract.contCurCd;
        let currencyOption = currencyCodes.map(function(currencyCode) {
            return { label : currencyCode.cdVal1, value : currencyCode.cdId }
        });
        ...
        $('.taxAplyTypeOpts').remove(); 
        let taxAplyTypeCodes = _COMMON_CODES.taxAplyTypeCodes;
        let defaultTaxAplyType = contract.taxAplyTypeCd;
        let taxAplyTypeOption = taxAplyTypeCodes.map(function(taxAplyTypeCode) {
            return { label : taxAplyTypeCode.cdVal1, value : taxAplyTypeCode.cdId }
        });
        
        initSingleSelectbox('#taxAplyType', 'taxAplyTypeOpts', taxAplyTypeOption, defaultTaxAplyType);

        let taxAplyPecnt = contract.taxAplyPecnt;
        $('#taxAplyPecnt').val(taxAplyPecnt);
        
        let excludeCallTypeCodes = _COMMON_CODES.excludeCallTypeCodes;
        let excludeCallTypeOption = excludeCallTypeCodes.map(function(excludeCallTypeCode) {
            return { label : excludeCallTypeCode.cdVal1, value : excludeCallTypeCode.cdId }
        });
        
        let defaultExcludeCall = contract.exceptAplyCall && contract.exceptAplyCall.split(',') || [];
        
        initMultiSelectbox('#excludeCalls', excludeCallTypeOption, defaultExcludeCall, 'excludeCalls');
        
        /** IB **/
        $('#contractBaseInfo').data('contId', contract.contId);
        $('#contractBaseInfo').data('contDtlId', contract.contDtlId);
        
    }
    
    function displayContractBaseTarifPanel(contract) {
        $('#newContBasTarifDiv').hide();
        $('#newContBasTarifs').empty();
        
        let basTarifType = 'basTarifCond';
        let contBasTarifs = contract.contBasTarifs;
        let basTarifHtml = '';
        
        contBasTarifs.forEach(contBasTarif => {
            //add used callTypes
            _BAS_CALL_TYPES.push(contBasTarif.callTypeCd);
            basTarifHtml += generateContBasTarifHtml(contBasTarif, _COMMON_CODES, basTarifType, _BAS_CALL_TYPES);
        });
    ...
    function displayContractSpecialModelPanel(contract) {
        $(document).find('.special-condition.cmit').remove();
        $(document).find('.special-condition.imsi').remove();
        
        if (_.isEmpty(contract.contSpclTarifs)) return;
        
        let contCurCd = contract.contCurCd;
        let contSpclTarifs = contract.contSpclTarifs;
        
        let cmitContSpclTarifs = contSpclTarifs.filter(contSpclTarif => contSpclTarif.modelTypeCd === 'CMIT');
        
        if (!_.isEmpty(cmitContSpclTarifs)) {
            _MSZI = _MSZI - 10;
            displaySpecialModelPanel("CMIT", cmitContSpclTarifs, contCurCd);
            $('.specialModelOpts').val('CMIT');
        }
        
        // 3. imsi cap models
        let imsiContSpclTarifs = contSpclTarifs.filter(contSpclTarif => contSpclTarif.modelTypeCd === 'IMSI');
        if (!_.isEmpty(imsiContSpclTarifs)) {
            _MSZI = _MSZI - 10;
            displaySpecialModelPanel("IMSI", imsiContSpclTarifs, contCurCd);
            $('.specialModelOpts').val('IMSI');
        }
        
    }
    
    
    function displaySpecialModelPanel(specialModel, contSpclTarifs, contCurCd) {
        if (_.isEmpty(contSpclTarifs)) return;
        
        ({ spclCondPanelHtml, spclCallTypeIds } = generatePrevSpecialCondPanel(specialModel, contSpclTarifs, contCurCd, _COMMON_CODES));
        
        $('#addSpecialModel').parents(".list-group").find(".inr ~ .btn-group").before(spclCondPanelHtml);
        ...
        contSpclTarifs.forEach((contSpclTarif, idx) => {
            let defaultCallTypeCodes = contSpclTarif.callTypeCd.split(",");
            initMultiSelectbox('#' + spclCallTypeIds[idx], callTypeCodeOption, defaultCallTypeCodes, 'spclCallType');
        });
    ...
    function displayNewBaseContractPanel(contId, contDtlId) {
        let url = _CTX + "dch/contract/findContractDetailByContIdAndConDtlId";
        let params = {};
        params.contId = contId;
        params.contDtlId = contDtlId;
        ...
        $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
            let result = response['data'];
            _CONT_DTL_DOMAIN = result;
            _CONT_DTL_DOMAIN.contDtlId = '';
            
            displayNewBaseContractBaseInfoPanel(result);
    ...
    // set start date
    function setContractNewStartDate(contStDate) {
        let startDatepicker = $('#contStDate').data('datepicker');
        startDatepicker.selectDate(addBugFixSubstractDays(contStDate));
        startDatepicker.selectDate(addBugFixAddDays(contStDate));
        // prevent selecting earlier than contract start day
    ...
    // set end date
    function setContractNewEndDate(contExpDate) {
        let endDatepicker = $('#contExpDate').data('datepicker');
        endDatepicker.selectDate(addBugFixSubstractDays(contExpDate));
        endDatepicker.selectDate(addBugFixAddDays(contExpDate));
    }
    
    
    // set end noti date
    function setContractNewEndNotiDate(contExpNotiDate) {
        let endNotiDatepicker = $('#contExpNotiDate').data('datepicker');
        endNotiDatepicker.selectDate(addBugFixSubstractDays(contExpNotiDate));
        endNotiDatepicker.selectDate(addBugFixAddDays(contExpNotiDate));
    }
    
    
    function addBugFixSubstractDays(date) {
        date.setDate(date.getDate() - 100);
        return date;
    }
    
    
    function addBugFixAddDays(date) {
        date.setDate(date.getDate() + 100);
        return date;
    }
    
    function displayNewBaseContractBaseInfoPanel(contract) {
        let contSttusCd = contract.contSttusCd;
        
        $('#contStDate').val(_m(addDays()).format('YYYY-MM-DD'));
        $('#contExpDate').val(_m(addDays()).format('YYYY-MM-DD'));
        $('#contExpNotiDate').val(_m(addDays()).format('YYYY-MM-DD'));
        
        if (contSttusCd === 'DRF' || contSttusCd === 'AGR') {
            let contStDate = _m(contract.contExpDate, ["YYYY-MM-DD"]).add(1, 'd').toDate();
            let contExpDate = _m(contStDate, ["YYYY-MM-DD"]).add(1, 'y').subtract(1, 'd').toDate();
            let contExpNotiDate = _m(contExpDate, ["YYYY-MM-DD"]).subtract(2, 'M').toDate();
            
            setContractNewEndDate(contExpDate);
            setContractNewEndNotiDate(contExpNotiDate);
            setContractNewStartDate(contStDate);
        } 
        else if (contSttusCd === 'DCL') {
            $('#contExpDate').data('datepicker').selectDate(_m(contract.contExpDate, ["YYYY-MM-DD"]).toDate());
            let startDatepicker = $('#contStDate').data('datepicker');
            startDatepicker.selectDate(_m(contract.contStDate, ["YYYY-MM-DD"]).toDate());
            startDatepicker.update({minDate : _m(contract.contStDate, ["YYYY-MM-DD"]).toDate()}); // prevent selecting earlier than contract start day
            $('#contExpNotiDate').data('datepicker').selectDate(_m(contract.contExpNotiDate, ["YYYY-MM-DD"]).toDate());
        }
        ...
        //let defaultExcludeCall = contract.exceptAplyCall.split(',');
        let defaultExcludeCall = contract.exceptAplyCall && contract.exceptAplyCall.split(',') || [];
    ...
    function initMultiSelectbox(element, options, values, dataName) {
        // set init value
        if (element == '#excludeCalls') {
            $(element).empty();
            $(element).html('<span>Exclude Calls</span>');
        }
        $(element).data(dataName, values);
        // render selectbox
        let sp = new SelectPure(element, {
    ...
    function validMinimumContractSubmit() {
        ...
        //if (!exceptAplyCall) {
        //    global.alert('Excluded Calls을 선택하세요.');
        //    return false;
        //}
    ...
    function assembleContractDomains(url, contSttusCd = 'WRK') {
        ...
        lastContSpclTarifs.forEach(lastContSpclTarif => {
            ...
            contSpclTarifs.forEach(contSpclTarif => {
                ...
                if (contSpclTarif.contSpclTarifId === lastContSpclTarif.contSpclTarifId) { // compare with temp id
    ...
    // set start date
    function setContractStartDate(element, days = 1) {
        $(element).data('datepicker').selectDate(addDays(days));
    }
    
    
    // set end date
    function setContractEndDate(element, days = 366) {
        $(element).data('datepicker').selectDate(addDays(days - 100)); // date select bugfix
        $(element).data('datepicker').selectDate(addDays(days));
    }
    
    
    // set end notification date
    function setContractEndNotiDate(element, days = 307) {
        $(element).data('datepicker').selectDate(addDays(days));
    }
    
    
    // set fixed rate settle date
    function setFixedRateDate(element, days = 0) {
        $(element).data('datepicker').selectDate(addDays(days));
    }
    
    
    // set special cond noti date
    function setSpecialCondNotiDate(element, days = 0) {
        $(element).data('datepicker').selectDate(addDays(days));
    }
    

    function createContractStartDatepicker() {
        ...
        $('#contExpDate').datepicker({
            ...
            onSelect: function (formatDate, date, picker) {
                ...
                if (startDate.isValid() && endDate.isSameOrBefore(startDate)) {
                    ...
                    picker.clear();
                    //picker.selectDate(startDate.add(366, 'd').toDate());
                }
        ...
        $('#contExpNotiDate').datepicker({
            ...
            onSelect: function (formatDate, date, picker) {
                ...
                if (notiDate.isValid() && (notiDate.isSameOrBefore(startDate) || notiDate.isSameOrAfter(endDate))) {
                    ...
                    picker.clear();
                    //picker.selectDate(endDate.subtract(60, 'd').toDate());
                }
            }
    ...
    function initContractCalendarDate() {
        setContractStartDate('#contStDate'); // contract start day
        setContractEndDate('#contExpDate'); // contract end day
        setContractEndNotiDate('#contExpNotiDate'); // contract end noti day
        setFixedRateDate('#fixAmtDate'); //fixed rate settle day
        setSpecialCondNotiDate('#spclNotiDate'); // special cond noti day
    }
    ...
    const thisModuleEventHandlers = function() {
        ...
        $('#addContBasDtlTarif').on('click', function(e) {
            ...
            if (_TAB_DIRECTION !== 'IB' || _CONTRACT_STATUS !== 'Working') return;
        ...
        $(document).on('click','.add-table + .table-box .add-type-button',function(e) {
            ...
            let contDtlBasTarifIdx = contBasTarifs.findIndex(contBasTarif => contBasTarif.contBasTarifId === tmpBasTarifId); // compare with temp id
        ...
        $(document).on('change', '.callTypeOpts', function(e) {
            ...
            if (callType.startsWith('M')) { // calltype is a kind of voice
                defaultAddFeeType = 'Call Setup Fee';
        ...
        $('#addSpecialModel').on('click', function(e) {
            ...
            let $newSpclCondTr =  $latestSpclPanel.find('tr').not(':first');
        ...
        $(document).on('click','.table-box.add-type .btn.delete',function() {
            ...
            tmpSpclCondIds.forEach(tmpSpclCondId => {
                let contSpclTarifsIdx = contSpclTarifs.findIndex(contSpclTarif => contSpclTarif.contSpclTarifId === tmpSpclCondId); // compare with temp id
        ...
        $(document).on('click','.table-box.special-condition .add-type-button.remove',function(e) {
            ...
            if($(this).hasClass("remove")) {
                ...
                if (spclConds > 1) {
                    ...
                    let contSpclTarifs = _CONT_DTL_DOMAIN.contSpclTarifs;
                    let contSpclTarifIdx = contSpclTarifs.findIndex(contSpclTarif => contSpclTarif.contSpclTarifId === tmpSpclCondId); // compare with temp id
        ...
        $(document).on('click', '.editSpclCond', function(e) {
            ...
            let contSpclTarifs = _CONT_DTL_DOMAIN.contSpclTarifs;
            let contSpclTarif = contSpclTarifs.find(contSpclTarif => contSpclTarif.contSpclTarifId === tmpSpclCondId); // compare with temp id
        ...
        $(document).on('click','.table-box.special-condition .add-type-button.add',function(e) {
            ...
            if(specialRows <= 7) {
                ...
                contSpclTarif.contSpclTarifId = tmpSpclCondId; // 임시 특수 조건 아이디
                contSpclTarif.modelTypeCd = ($(this).closest('tr').find('td input[name="modelTypeCd"]').val() === 'Commitment') && 'CMIT' || 'IMSI';
                contSpclTarif.stepNo = $(this).closest('tr').find('td input[name="stepNo"]').val();
                let callTypeId = $(this).closest('tr').find('div.spclCallType').attr('id');
                let callTypes = getValueFromElement('#' + callTypeId, 'spclCallType');
                ...
                $multiSelect.each(function(idx){
                    $(this).css('z-index', firstSelectIndex - ++idx);
                });
        ...
        $('#saveSpclModal').on('click', function(e) {
            ...
            let contSpclTarifs = _CONT_DTL_DOMAIN.contSpclTarifs;
            let contSpclTarif = contSpclTarifs.find(contSpclTarif => contSpclTarif.contSpclTarifId === _TMP_SPCL_ID); // compare with temp id
        ...
        $(document).on('click', 'tbody#contractList > tr', function(e) {
            _DATE_PICKER_INIT = false;
            
            $('#contractList').find('tr').each(function() {
            ...
            let contDtlId = $(this).data('contDtlId');
            _MSZI = 99;
            
            _TAB_DIRECTION = $(this).find('td').eq(0).text();
            _CONTRACT_STATUS = $(this).find('td').eq(1).text();
            
            changeTabTitleButtonDisplay(_TAB_DIRECTION, _CONTRACT_STATUS);
            
            findContractDetailByContIdAndConDtlId(contId, contDtlId);
            
            // show contract detail divs
            showContractDetailDivs();
        ...
        $('#newContract').on('click', function(e) {
            ...
            $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
                let result = response['data'];
                let contracts = result.contDtls
                
                let workingContract = contracts.find(contract => contract.direction === 'IB' && contract.contSttusCd === 'WRK');
                let draftContract = contracts.find(contract => contract.direction === 'IB' && contract.contSttusCd === 'DRF');
                
                if (!_.isEmpty(workingContract) || !_.isEmpty(draftContract)) {
                    global.alert(thisPage.appMsg031);
                    return;
                }
                
                let baseNewContract = contracts.find(contract => contract.direction !== 'OB' && contract.contSttusCd !== 'WRK');
                if (!_.isEmpty(baseNewContract)) {
                    
                    displayNewBaseContractPanel(baseNewContract.contId, baseNewContract.contDtlId);
                    ...
                    // change tab name
                    $('#contTypeDirection').text('Creating New IB Contract');
                    
                    let contracts = $(document).find('#contractList').find('tr');
                    
                    // contract list selected off
                    contracts.each(function() {
                        $(this).removeClass('selected');
                    });
                    
                    // contract list selected on
                    contracts.each(function(i) {
                        let direction = $(this).find('td:eq(0)').text();
                        let status = $(this).find('td:eq(1)').text();
                        
                        if (direction === 'IB' && (status === 'Rejected' || status === 'Agreed')) {
                            $(this).addClass('selected');
                            return false;
                        }
                    });
                    ...
                    global.alert(thisPage.appMsg033);
                    global.location.href = _CTX + "dch/contract/createContract"; 
                }
        ...
        $('#saveTempContract').on('click', function(e) {
            let url = _CTX + "dch/contract/saveTemporaryContract";
            let contDtlId = _CONT_DTL_DOMAIN.contDtlId;
            let contSttusCd = _CONT_DTL_DOMAIN.contSttusCd;
            
            if (!contDtlId && contSttusCd === 'WRK') {
            ...
            let params = assembleContractDomains(url, contSttusCd);
            
            if (!params) {
                _CONT_DTL_DOMAIN.contSttusCd = '';
                return;
            }
        ...
        $('#requestConsent').on('click', function(e) {
            let url = _CTX + "dch/contract/requestAgreementForInboudContract";
            let requestAgree = false;
            
            $('#contractList tr').each(function() {
                let direction = $(this).find('td:eq(0)').text();
                let contSttus = $(this).find('td:eq(1)').text();
                if (direction === 'IB' && contSttus === 'Draft') {
                    requestAgree = true;
                    return false;
                }
            });
            ...
            let confirm = global.confirm(thisPage.appMsg038);
            if (!confirm) return;
            ...
            $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
                ...
                if (result.statusCodeValue === 200) {
                    global.alert(thisPage.appMsg039);
                } else {
                    global.alert(thisPage.appMsg040);
                }
                global.location.reload();
            });
        ...
        $('#responseConset').on('click', function(e) {
            // confirm before request
            let confirm = global.confirm(thisPage.appMsg041);
            if (!confirm) return;
            
            let params = _CONT_DTL_DOMAIN;
            let url = _CTX + "dch/contract/agreeOutboundConatract";
            
            $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
                let result = response['data'];
                if (result.statusCodeValue === 200) {
                    global.alert(thisPage.appMsg042);
                } else {
                    global.alert(thisPage.appMsg043);
                }
                global.location.reload();
            });
        ...
        $('#responseRefusal').on('click', function(e) {
            // confirm before request
            let confirm = global.confirm(thisPage.appMsg044);
            if (!confirm) return;
            
            let params = _CONT_DTL_DOMAIN;
            let url = _CTX + "dch/contract/disagreeOutboundContract";
            
            $.ajaxProxy($.reqPost(url).setParams(params).build()).done(function(response) {
                let result = response['data'];
                if (result.statusCodeValue === 200) {
                    global.alert(thisPage.appMsg045);
                } else {
                    global.alert(thisPage.appMsg046);
                }
                global.location.reload();
            });
    ...
    const thisModuleInitializr = function() {
        alertReturnMessage();
        findCommonCodes();
        initDisplayContractDetailListByPlmns();
        createContractStartDatepicker();
