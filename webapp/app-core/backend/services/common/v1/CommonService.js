...
const AbstractService = require(global.GLOBAL_BACKEND_ROOT +
  '/services/AbstractService');
const ensureAuthenticated = require(global.GLOBAL_BACKEND_ROOT +
  '/libs/middlewares').ensureAuthenticated;
...
class CommonService extends AbstractService {
  ...
  converToPrivateDocument(document) {
    ...
    this.getLogger().debug(
      '[CommonService::processDocument] documentData %s ',
      documentData,
    );

    const documentDataJson = JSON.parse(documentData);
    this.getLogger().debug(
      '[CommonService::processDocument] documentDataJson %s ',
      JSON.stringify(documentDataJson),
    );
  ...
  async processPrivateDocument(privateDocument) {
    this.getLogger().debug(
      '[CommonService::processDocument] processing documentId %s ',
      privateDocument.documentId,
    );

    if (
      await this.getBackendAdapter('localStorage').existsDocument(
        privateDocument.documentId,
      )
    ) {
      const data = {
      ...
      await this.getBackendAdapter('localStorage').updateDocument(
        privateDocument.documentId,
        data,
      );
    } else {
      await this.getBackendAdapter('localStorage').storeDocument(
        privateDocument.documentId,
        privateDocument,
      );
      this.getLogger().info(
        '[CommonService::privateDocument] Stored document with id %s successfully',
        privateDocument.documentId,
      );
    }
  }
  ...
  registerRequestHandler() {
      ...
      async (req, res) => {
        ...
        try {
          ...
          const response = requireRaw
            ? await this.getBackendAdapter('common').getRawContractById(
                contractId,
              )
            : await this.getBackendAdapter('common').getContractById(
                contractId,
                this.mspid,
              );
          return res.json(response);
    ...
    this.getRouter().post(
      ...
      async (req, res) => {
        try {
          ...
          const response = await this.getBackendAdapter(
            'common',
          ).createContract(toMSP, data);
          return res.json(response);
        } catch (error) {
          this.getLogger().error(
            '[CommonService::/] Failed to store document - %s',
            error.message,
          );
          this.handleError(
            res,
    ...
    this.getRouter().get(
      ...
      async (req, res) => {
        const contractId = req.params.contractId;
        try {
          const response = await this.getBackendAdapter('common').getSignatures(
            contractId,
          );
          return res.json(response);
        } catch (error) {
    ...
    this.getRouter().put(
      ...
      async (req, res) => {
        const contractId = req.params.contractId;
        try {
          const identity = await this.getBackendAdapter('wallet').getIdentity(
            req.user.enrollmentId,
          );
          const privateKey = identity.credentials.privateKey;
          const certificate = identity.credentials.certificate;
          const document = await this.getBackendAdapter(
            'common',
          ).getRawContractById(contractId);
          const signature = cryptoUtils.createSignature(
            privateKey,
            document.data,
          );
          const signatureAlgo = 'ecdsa-with-SHA256_secp256r1';
          const response = await this.getBackendAdapter('common').signContract(
            contractId,
            certificate,
            signatureAlgo,
            signature,
     ...
     */
    this.getRouter().get(
      '/signatures/:contractId/:signatureId',
      ensureAuthenticated,
      async (req, res) => {
        const contractId = req.params.contractId;
        ...
        try {
          const response = await this.getBackendAdapter(
            'common',
          ).getSignaturesById(contractId, signatureId);
          return res.json(response);
        } catch (error) {
          this.handleError(
            res,
    ...
    this.getRouter().get(
      ...
      async (req, res) => {
        try {
          const contractId = req.params.contractId;
          const response = await this.getBackendAdapter('common').getUsages(
            contractId,
          );
          return res.json(response);
        } catch (error) {
     ...
     */
    this.getRouter().get(
      '/usages/:contractId/:usageId',
      ensureAuthenticated,
      async (req, res) => {
        try {
          const contractId = req.params.contractId;
          const usageId = req.params.usageId;
          const response = await this.getBackendAdapter('common').getUsagesById(
            contractId,
            usageId,
          );
          return res.json(response);
    ...
    this.getRouter().post(
      ...
      async (req, res) => {
        try {
          const contractId = req.params.contractId;
          const data = req.body;
          const response = await this.getBackendAdapter('common').createUsage(
            contractId,
            data,
          );
          return res.json(response);
          ...
          this.getLogger().error(
            '[CommonService::/] Failed to store usage - %s',
            error.message,
          );
          this.handleError(
            res,
    ...
    this.getRouter().put(
      ...
      async (req, res) => {
        ...
        const data = req.body;
        try {
          const response = await this.getBackendAdapter('common').updateUsage(
            contractId,
            usageId,
            data,
          );
    ...
    this.getRouter().delete(
      ...
      async (req, res) => {
        ...
        const usageId = req.params.usageId;
        try {
          const response = await this.getBackendAdapter('common').deleteUsage(
            contractId,
            usageId,
          );
          return res.json(response);
     ...
     */
    this.getRouter().put(
      '/usages/:contractId/:usageId/generate/',
      ensureAuthenticated,
      async (req, res) => {
        const contractId = req.params.contractId;
        const usageId = req.params.usageId;
        try {
          const response = await this.getBackendAdapter(
            'common',
          ).generateSettlementsById(contractId, usageId);
          return res.json(response);
        } catch (error) {
          this.handleError(
            res,
     ...
     */
    this.getRouter().get(
      '/settlements/:contractId/:settlementId',
      ensureAuthenticated,
      async (req, res) => {
        try {
          const contractId = req.params.contractId;
          ...
          const response = await this.getBackendAdapter(
            'common',
          ).getSettlementsById(contractId, settlementId);
          return res.json(response);
        } catch (error) {
          this.handleError(
            res,
     ...
     */
    this.getRouter().put(
      '/settlements/:contractId/:settlementId/send/',
      ensureAuthenticated,
      async (req, res) => {
        const contractId = req.params.contractId;
        const settlementId = req.params.settlementId;
        try {
          const response = await this.getBackendAdapter(
            'common',
          ).sendSettlementsById(contractId, settlementId);
          return res.json(response);
        } catch (error) {
          this.handleError(
            res,
