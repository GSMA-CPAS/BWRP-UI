  ...
  registerRequestHandler() {
    ...
    this.getRouter().post("/", ensureAuthenticated, async (req, res) => {
      try {
        ...
        const documentDataBase64 = new Buffer.from(documentData).toString(
          "base64"
        );
        const response = await this.getBackendAdapter(
          "blockchain"
        ).uploadPrivateDocument(toMSP, documentDataBase64);
        return res.json(response);
      } catch (error) {
        this.getLogger().error(
          "[DocumentService::/] Failed to store document - %s",
          error.message
        );
        return errorHandler(
          res,
    ...
    this.getRouter().post("/event", async (req /*, res*/) => {
      ...
      if (body && body.data && body.data.documentID) {
        ...
        try {
          const documentData = await this.getBackendAdapter(
            "blockchain"
          ).getPrivateDocument(documentId);
          ...
          if (documentData) {
            ...
            this.getLogger().info(
              "[DocumentService::/event] Stored document with id %s successfully",
              documentId
            );
          } else {
            this.getLogger().error(
              "[DocumentService::/event] Failed to get document with id - %s",
              documentId
            );
          }
        } catch (error) {
          this.getLogger().error(
            "[DocumentService::/event] Failed to store document - %s",
            error.message
          );
        }
      }
