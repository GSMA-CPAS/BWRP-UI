    ...
    fetchTwoFactorStatus() {
      this.$http({method: 'get', url: '/api/v1/users/2fa/status', withCredentials: true}).then((response) => {
        ...
        if (this.twoFactorIsEnabled) {
          ...
          this.twoFactorMainText = 'Two-factor authentication adds an extra layer of protection to the login process. Once enabled and configured, each time you sign in, you will ' +
              'be asked to enter an additional security code generated by your authenticator app (such as Google Authenticator or Authy).';
