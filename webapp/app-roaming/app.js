...
exports.init = async (app, router, database, logger, config) => {
  ...
  router.get('/tadig/codes', async (req, res) => {
    try {
      ...
      logger.error('[app-roaming::GET/tadig/codes] failed to query tadig codes - %s', error.message);
  ...
  router.post('/tadig/codes', async (req, res) => {
    try {
      ...
      logger.error('[app-roaming::POST/tadig/codes] failed to insert tadig code - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
    }
  });
  ...
  router.delete('/tadig/codes/:id', async (req, res) => {
    ...
    try {
      ...
      logger.error('[app-roaming::POST/tadig/codes] failed to delete tadig code - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
    }
  });
  ...
  router.get('/tadig/groups', async (req, res) => {
    try {
      ...
      logger.error('[app-roaming::GET/tadig/groups] failed to query tadig groups - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
    }
  });
  ...
  router.get('/tadig/groups/:id', async (req, res) => {
    ...
    try {
      const result = await database.query('SELECT tc.code, tgr.tadig_code_id FROM tadig_groups_relation tgr INNER JOIN tadig_codes tc ON tgr.tadig_code_id=tc.id WHERE tgr.tadig_group_id=?', [tadigGroupId]);
      res.json(result);
    } catch (error) {
      logger.error('[app-roaming::GET/tadig/groups] failed to query tadig groups - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
  ...
  router.post('/tadig/groups', async (req, res) => {
    try {
      ...
      logger.error('[app-roaming::POST/tadig/groups] failed to insert tadig group - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
    }
  });
  ...
  router.post('/tadig/groups/:id/codes', async (req, res) => {
    ...
    try {
      await database.query('INSERT INTO tadig_groups_relation (tadig_group_id, tadig_code_id) VALUES ?', [values]);
      res.json({success: true});
    } catch (error) {
      logger.error('[app-roaming::POST/tadig/groups] failed to insert tadig group code relation - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
    }
  });
  ...
  router.delete('/tadig/groups/:id/codes', async (req, res) => {
    ...
    try {
      await database.query('DELETE FROM tadig_groups_relation WHERE tadig_group_id=? AND tadig_code_id IN (?)', [tadigGroupId, values]);
      res.json({success: true});
    } catch (error) {
      logger.error('[app-roaming::POST/tadig/groups] failed to insert tadig group code relation - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
  ...
  router.delete('/tadig/groups/:id', async (req, res) => {
    ...
    try {
      ...
      logger.error('[app-roaming::POST/tadig/groups] failed to delete tadig group - %s', error.message);
      res.status(500).json({status: 500, code: 'ERR_INTERNAL_SERVER_ERROR', message: 'Internal Server Error'});
    }
  });
  ...
  try {
    ...
    if (error.errno === 1146) {
      try {
        ...
        logger.info('[app-roaming::init] table %s has been created successfully!', tableName);
      } catch (error) {
        logger.error('[app-roaming::init] failed to create %s table - %s ', tableName, JSON.stringify(error));
      }
    } else {
      logger.error('[app-roaming::init] Error checking database - %s ', JSON.stringify(error));
    }
  }
};
