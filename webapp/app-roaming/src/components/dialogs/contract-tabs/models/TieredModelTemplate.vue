<template>
  <v-col  class="striped-column-container">
    <v-row>
      <v-col class="striped-column"><b>{{ modelType }}</b></v-col>
      ...
      <v-col class="striped-column"></v-col>
      <v-col class="striped-column"><v-icon v-if="inCommitment" color="primary">
    ...
    >
      ...
      <v-col class="striped-column">{{ `${start} ➜ ${(index + 1) === data.ratingPlan.rate.thresholds.length ? 'Unlimited' : data.ratingPlan.rate.thresholds[index+1].start}` }}</v-col>
      <v-col class="striped-column">{{ Number(fixedPrice || 0).toLocaleString() }}</v-col>
      <v-col class="striped-column">{{ Number(linearPrice || 0).toLocaleString() }}</v-col>
      <v-col class="striped-column"></v-col>
      <!--<v-col>
