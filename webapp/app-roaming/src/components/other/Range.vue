<template>
  <v-row dense class="range-container">
    <div v-if="label" class="range-label" :class="{'primary--text':focused}">{{label}}</div>
    <v-col cols="4">
      <v-text-field v-on="listeners" v-model="range.from" @input="handleInput" label="from"></v-text-field>
    </v-col>
    <v-col align-self="center" class="range-seperator" cols="1">
      <hr />
    </v-col>
    <v-col cols="4">
      <v-text-field v-on="listeners" v-model="range.to" label="to"></v-text-field>
    </v-col>
  </v-row>
</template>
<script>
