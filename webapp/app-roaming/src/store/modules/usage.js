...
const usageModule = {
  ...
  actions: {
    ...
    async getUsages(
      ...
      await Vue.axios.commonAdapter
        ...
        .then((data) => {
          ...
          if (data[0] && data[0].tag === 'REJECTED') {
            ...
            dispatch(
              'timelineCache/updateCacheField',
              {
                ...
                field: 'usageId',
                newValue: currentUsage.usageId,
              },
              {root: true},
            );
            dispatch('getUsageById', {
              contractId,
              usageId: currentUsage.usageId,
              isPartner: false,
            });
          ...
          commit(
            'timelineCache/UPDATE_USAGE_LIST',
            data.filter((usage) => {
          ...
          if (currentUsage.partnerUsageId) {
            ...
            dispatch(
              'timelineCache/updateCacheField',
              {
                usageId: currentUsage.usageId,
                field: 'partnerUsage',
                newValue: {id: currentUsage.partnerUsageId},
              },
              {root: true},
            );
          }
    ...
    async getUsageById(
      ...
      await Vue.axios.commonAdapter
        ...
        .then((data) => {
          ...
          if (req.isPartner) {
            ...
            if (settlementId) {
              commit('settlement/UPDATE_PARTNER_SETTLEMENT_ID', settlementId, {
                root: true,
              });
              if (req.cacheItemId) {
                dispatch(
            ...
            dispatch(
              'timelineCache/updateCacheField',
              {usageId: usageId, field: 'ownUsage', newValue: data},
              {root: true},
            );
            if (settlementId) {
              commit('settlement/UPDATE_OWN_SETTLEMENT_ID', settlementId, {
                root: true,
              });
              dispatch(
                'timelineCache/updateCacheField',
      ...
      if (
        state.ownUsage?.id &&
        state.ownUsage?.state === 'SENT' &&
        state.partnerUsage?.id
      ) {
      ...
      await Vue.axios.commonAdapter
        ...
        .then((data) => {
          ...
          dispatch(
            'timelineCache/updateCacheField',
            {
              usageId: state.ownUsage.id,
              field: 'usageDiscrepancies',
              newValue: data,
            },
            {root: true},
          );

          if (ownSettlementId && partnerSettlementId) {
            dispatch('settlement/getSettlementDiscrepancies', contractId, {
              root: true,
            });
          }
        })
    ...
    async uploadUsage(
      ...
      await Vue.axios.commonAdapter
        ...
        .then((res) => {
          const {usageId, state, body, creationDate} = res;
          commit('UPDATE_USAGE', {
            id: usageId,
            state: state,
            body: body,
          ...
          dispatch(
            'timelineCache/updateCacheField',
            {usageId: usageId, field: 'usageId', newValue: usageId},
            {root: true},
          );
          dispatch(
            'timelineCache/updateCacheField',
            {usageId: usageId, field: 'ownUsage', newValue: res},
            {root: true},
          );
        })
        .catch((err) => {
    ...
    async getUsageSignatures(
      ...
      commit('setLoading', true);
      await Vue.axios.commonAdapter
        .get(`/usages/` + contractId + '/' + ownUsageId + '/signatures/', {
          customLoader: true,
        })
        .then((data) => {
          partnerSignature = data
            .filter(
              (signature) =>
                signature.state === 'SIGNED' && signature.msp === partnerMsp,
            )
            .pop();
      ...
      await Vue.axios.commonAdapter
        .get(`/usages/` + contractId + '/' + partnerUsageId + '/signatures/', {
          customLoader: true,
        })
        .then((data) => {
          ownSignature = data
            .filter(
              (signature) =>
                signature.state === 'SIGNED' && signature.msp === selfMsp,
            )
            .pop();
